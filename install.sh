...
if [ -z "$FOUND_IN_SOURCES_CONF" ]; then
  cat $SOURCES_CONF | fgrep -v "$DEFAULT_SOURCES_LINE1" | fgrep -v "$DEFAULT_SOURCES_LINE2" > $SOURCES_CONF.tmp
  mv $SOURCES_CONF.tmp $SOURCES_CONF
  echo "$SOURCES_CONF_LINE" >> $SOURCES_CONF
  echo "$DEFAULT_SOURCES_LINE1" >> $SOURCES_CONF
  echo "$DEFAULT_SOURCES_LINE2" >> $SOURCES_CONF
fi
